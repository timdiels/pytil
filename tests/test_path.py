...
def assert_file_mode(path, mode):
    ...
    assert actual == mode, f'{actual:o} != {mode:o}'
...
class TestDigestDirectory:
    ...
    def test_no_cwd(self, root, original):
        ...
        try:
            ...
            # Restore cwd for next tests, this should actually be in a fixture
...
class TestTemporaryDirectory:
    ...
    def cleanup(self, monkeypatch):
        mock = Mock()
        monkeypatch.setattr(tempfile.TemporaryDirectory, 'cleanup', mock)
        return mock
